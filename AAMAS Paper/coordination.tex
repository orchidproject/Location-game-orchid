...
Algorithm~\ref{alg:pathplanning}. We first compute the $Q$ value
for each action (Line 3-4) and select the best action (Line 5).
Then, we update the value function (Line 6) and simulate the next
state (Line 7). This process repeats until the goal state is
reached (Line 9). If the goal is not reached, we assume there does
not exist a path between the start location of the responder and
the location of the task (either there are obstacles on the path or
the responder will be killed by the radioactivity on the road).
