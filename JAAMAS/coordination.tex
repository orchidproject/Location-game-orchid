...
\begin{algorithm}[htbp]
  ...
  and a set of free FRs $I$.}
  \KwOut{a task assignment for all FRs.}
  \Indp\BlankLine
  ...
  \ForEach{$C_{jk} \in \{C_{jk}\}$}{
    ...
    \For{$i=1$ \KwTo $N$}{
        $(r, s') \gets$ run the simulation starting at state $s$
        until task $k$ is completed by the FRs in $C_{jk}$ \;
