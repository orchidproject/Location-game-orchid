...
\item \textit{Explicit reject}. On rejection of a task by any of the first responders, a new plan is requested. The rejected allocation is used to constrain the set of solutions returned. For example, if two responders (a medic and a soldier) were allocated a task and the medic rejected it, the planning agent would search for an  allocation with the constraint that this medic should not be allocated this task.
\end{itemize} 
