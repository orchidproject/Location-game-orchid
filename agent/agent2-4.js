...
var ubiclient = require('./agent_ubiserver.js') ;
...
function mainloop(){    
    ...
    truckUpdateID=setInterval(function(){
    	var state = ubiclient.getsenderstate();
    	state.begin();
    	state.set('latitude',helper.player.lat);
    	state.set('longitude',helper.player.lng);
    	state.end();
    ...
    //set initial postion
     helper.player.lat=52.9523331;
    helper.player.lng=-1.1863445;

    //move a truck along a path
    var path=[helper.player,{lat:52.9529583,lng:-1.186727},{lat:52.9540927,lng:-1.18750480},{lat:52.9544232,lng:-1.189549}];
    var section=0;
...
helper.join('agent','a@agent.com','truck',1,'AA', function(p){
    ...
    if (p.user_id != null){
        ...
        ubiclient.connect('http://localhost:49891', 
        	p.user_id , 
        	"agent", 
        	"locations", 
			"locations-"+game_id, 
		...
		var state = ubiclient.getsenderstate();
		state.begin();
		state.set('skill',helper.player.skill);
		state.set('initials',helper.player.initials);
		state.end();
