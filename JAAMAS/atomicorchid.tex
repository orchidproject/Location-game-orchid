...
\todo{COMMENT FROM HAIDM WORKSHOP REVIEW: the discussion of the atomic orchid game suggests that stress is created in the simulation. The human factors little distinguishes between stress and workload, so it would be  accurate to say that the game induces workload but it's impact on stress is unknown or unverified.}
\todo{COMMENT FROM HAIDM: -- explain why the responders can't see the expanding radioactivity cloud. This seems to be a big design error to have data that is not shared with people who rely on that data to make safe decisions.}

\todo{JOEL/WENCHAO: Please add back any text you think may help clarify the system and the key results and relationships to past work}
